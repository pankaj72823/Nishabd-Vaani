...
def main():
    ...
    while True:
        ...
        results = hands.process(image)  #mediapipe stores handmark infomation
        ...
        if results.multi_hand_landmarks is not None and len(results.multi_hand_landmarks) == 2:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                if hand_sign_id == 'random':  # Point gesture
...
def select_mode(key, mode):
    ...
    if 48 <= key <= 57:  # ASCII values for '0' to '9'
        digit = key - 48
        number = current_number * 10 + digit
    if key == 110:  # n
        mode = 0
    if key == 107:  # k
        mode = 1
...
def draw_info_text(image, brect, handedness, finger_gesture_text, hand_sign_text):
